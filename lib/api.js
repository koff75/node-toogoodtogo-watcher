...
function loginByEmail(){
    ...
    return api.post('auth/v1/loginByEmail', {
        json: {
            device_type: "UNKNOWN",
...
function refreshToken(){
    ...
    return api.post('auth/v1/token/refresh', {
        json: {
...
function listBusinesses(favorites){
    const session = getSession();
    const localization = config.get('localization');
        
    return api.post('item/v4/', {
        json: {
