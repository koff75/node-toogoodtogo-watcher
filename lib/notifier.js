...
const { iif, from, of, interval, combineLatest, timer } = require('rxjs');
...
function pollBusinesses$(enabled$, favorite_only){
    ...
    return listFavoriteBusinessesByInterval$(authenticationByInterval$, enabled$, favorite_only);
}
...
function listFavoriteBusinessesByInterval$(authenticationByInterval$, enabled$, favorite_only){
    ...
    return combineLatest(
        ...
        mergeMap(v =>
            iif(
                () =>  pollingStatus(favorite_only), // Parameter to know which poll to stop
                       of(v),
            )
        ),
        filter(([enabled]) => enabled),
        mergeMap(() => from(api.listBusinesses(favorite_only)).pipe(
            retry(2),
            catchError(logError),
