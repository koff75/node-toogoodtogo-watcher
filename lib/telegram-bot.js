...
function searchMarket(context) {
  context.reply(`- Search activated -
  Ecrire OK pour démarrer.`)

  const superWizard = new WizardScene(
    "super-wizard",
    ctx => {
      ctx.reply("Saisir le nom à surveiller :")
      return ctx.wizard.next()
    },
    ctx => {
      ctx.reply(`✅ Notifications based on ${ctx.message.text}`)
