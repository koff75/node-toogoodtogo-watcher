...
function hasSearchValue$() {
  const options = config.get("searching")
  return combineLatest(of(options.enabled), telegramBot.hasActiveChats$()).pipe(
    map(enabledItems => _.some(enabledItems))
  )
}
function notifyOnSearch(businesses) {
  // Je parcours les items par nom et s’il existe je requête avec l’ID spécifié pour récupérer les quantités.
}
/* === END === */
